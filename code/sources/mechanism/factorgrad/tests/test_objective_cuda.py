...
def main():
    ...
    uncorrected_time_result = factorgrad_loss(
        image,
        target,
        1.0,
        selected_decision,
    ...
    uncorrected_time_result.objective.backward()
    uncorrected_time_gradient_error = float(
        (image.grad - baseline_gradient).abs().max()
    )
    image.grad = None

    selected_channel_result = factorgrad_loss(
        image,
        target,
        1.0,
    ...
    selected_channel_result.objective.backward()
    uncorrected_channel_nonzero = int(
        torch.count_nonzero(image.grad.reshape(3, -1).abs().sum(1)).item()
    )
    image.grad = None
    ...
    report = {
        ...
        "uncorrected_time_gradient_error": uncorrected_time_gradient_error,
        "uncorrected_channel_nonzero": uncorrected_channel_nonzero,
        "channels": [
    ...
    if report["uncorrected_time_gradient_error"] > 2e-5:
        raise SystemExit("uncorrected time-gradient check failed")
    if report["uncorrected_channel_nonzero"] != 1:
        raise SystemExit("uncorrected channel-gradient sparsity check failed")
