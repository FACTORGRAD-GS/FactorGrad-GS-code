...
class GaussianModel:
    ...
    def densify_and_prune_fastgs(self, max_screen_size, min_opacity, extent, radii, args, importance_score = None, pruning_score = None):
        ...
        '''
        count_before = int(self.get_xyz.shape[0])
        ...
        clone_count = int(torch.count_nonzero(torch.logical_and(metric_mask, all_clones)).item())
        split_count = int(torch.count_nonzero(torch.logical_and(metric_mask, all_splits)).item())
        ...
        if remove_budget:
            ...
            prune_applied = int(torch.count_nonzero(final_prune).item())
        ...
        self.last_topology_event = {
            "event": "densify_and_prune",
            "before": count_before,
            "after": int(self.get_xyz.shape[0]),
            "clone_parents": clone_count,
            "split_parents": split_count,
            "split_children": 2 * split_count,
            "prune_candidates": int(to_remove.item()),
    ...
    def final_prune_fastgs(self, min_opacity, pruning_score = None):
        ...
        self.last_topology_event = {
            ...
            "before": count_before,
            "after": int(self.get_xyz.shape[0]),
            "opacity_candidates": int(torch.count_nonzero(prune_mask).item()),
            "consistency_candidates": int(torch.count_nonzero(scores_mask).item()),
            "prune_applied": prune_applied,
        }
