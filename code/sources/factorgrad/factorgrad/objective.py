...
class _ExactValueSelectedChannelSSIM(torch.autograd.Function):
    @staticmethod
    def forward(ctx, image, target, selected_channel, temporal_weight):
    ...
    def backward(ctx, output_gradient):
        ...
        # Dividing by CH=1 elements instead of RGB elements supplies the
        # Monte Carlo channel factor 3. temporal_weight supplies 1 / q_t.
        map_gradient = torch.ones_like(selected_image) * (
            output_gradient * ctx.temporal_weight / selected_image.numel()
        )
        ...
        return image_gradient, None, None, None
...
def factorgrad_loss(
    ...
    if decision.mode in ("baseline_full", "full_rgb"):
        ...
        structural_ssim = _fused(batched_image, batched_target, train=True)
        exact_dssim = 1.0 - structural_ssim
        objective_dssim = exact_dssim.detach() + decision.temporal_weight * (
