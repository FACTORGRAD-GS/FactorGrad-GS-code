...
FACTORGRAD_PACKAGE = os.path.join(WORKSPACE_ROOT, "factorgrad")
if FACTORGRAD_PACKAGE not in sys.path:
    sys.path.insert(0, FACTORGRAD_PACKAGE)
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, websockets, rl_controller_path=None, wandb_run=None, factorgrad_args=None):
    ...
    factorgrad = FactorGradController(
        enabled=factorgrad_args.factorgrad_enable,
        topology_end_iteration=opt.densify_until_iter,
        stabilization_steps=factorgrad_args.factorgrad_stabilization_steps,
        frozen_probability=factorgrad_args.factorgrad_probability,
        seed=factorgrad_args.factorgrad_seed,
        schedule=factorgrad_args.factorgrad_schedule,
        early_start_iteration=factorgrad_args.factorgrad_early_start_iteration,
    )
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        if factorgrad_args.factorgrad_enable:
            ...
            ssim_value = 1.0 - factorgrad_result.exact_dssim
        ...
        with torch.no_grad():
            ...
            # Log and save
            training_report(tb_writer, wandb_run, iteration, Ll1, loss, l1_loss, iter_time, testing_iterations, scene, render_fastgs, (pipe, background, opt.mult))
            if wandb_run:
                wandb_run.log({
    ...
    stats_to_save["peak_gpu_memory_allocated_mb"] = peak_memory
    stats_to_save["peak_gpu_memory_reserved_mb"] = peak_reserved
    
    with open(os.path.join(dataset.model_path, "training_statistics.json"), "w") as f:
        json.dump(stats_to_save, f, indent=4)
    factorgrad.save(dataset.model_path)

    if wandb_run:
        for k, v in stats_to_save.items():
            if isinstance(v, dict):
                wandb_run.summary[f"time/{k}_s"] = v["time_s"]
                wandb_run.summary[f"time/{k}_percent"] = v["percentage"]
            else:
...
def init_wandb(args):
    ...
    run_name = args.wandb_name or os.path.basename(os.path.normpath(args.model_path))
    ...
    if iteration in testing_iterations:
        ...
        for config in validation_configs:
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                if wandb_run:
                    wandb_run.log({
                        f"{config['name']}/l1_loss": l1_test.item(),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--wandb_project", type=str, default="LeGS")
    parser.add_argument("--wandb_name", type=str, default=None)
    parser.add_argument("--factorgrad_enable", action="store_true", default=False)
    parser.add_argument("--factorgrad_schedule", choices=("late", "full"), default="full")
    ...
    parser.add_argument("--factorgrad_early_start_iteration", type=int, default=3000)
