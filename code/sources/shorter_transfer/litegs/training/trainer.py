...
def start(lp:arguments.ModelParams,op:arguments.OptimizationParams,pp:arguments.PipelineParams,dp:arguments.DensifyParams,
          ...
          fp:arguments.FactorGradParams=None):

    if fp is None:
        fp = arguments.FactorGradParams.get_class_default_obj()
    os.makedirs(lp.model_path, exist_ok=True)
    tb_writer = SummaryWriter(lp.model_path) if TENSORBOARD_FOUND else None
    ...
    topology_end_iteration = dp.densify_until * op.iterations_per_epoch
    factorgrad = FactorGradController(
        enabled=fp.factorgrad_enable,
        topology_end_iteration=topology_end_iteration,
        stabilization_steps=fp.factorgrad_stabilization_steps,
        frozen_probability=fp.factorgrad_probability,
        seed=fp.factorgrad_seed,
        schedule=fp.factorgrad_schedule,
        early_start_iteration=fp.factorgrad_early_start_iteration,
        channel_sampling=True,
        temporal_sampling=True,
        topology_protection=True,
    )
    print(
        "FactorGrad-GS: enabled={} schedule={} topology_end_iteration={} "
        "stabilization={} probability={}".format(
            fp.factorgrad_enable,
            fp.factorgrad_schedule,
            topology_end_iteration,
            fp.factorgrad_stabilization_steps,
            fp.factorgrad_probability,
        )
    )
    ...
    for epoch in range(start_epoch,total_epoch):
        ...
        with StatisticsHelperInst.try_start(epoch):
            ...
            for step_index in range(op.iterations_per_epoch):
                global_iteration = epoch * op.iterations_per_epoch + step_index + 1
                ...
                factorgrad_decision = factorgrad.decision(global_iteration)
                if fp.factorgrad_enable:
                    factorgrad_result = factorgrad_loss(
                        img[0],
                        gt_image[0],
                        op.lambda_dssim,
                        factorgrad_decision,
                        importance_correction=not fp.factorgrad_no_importance_correction,
                    )
                    l1_loss = factorgrad_result.l1
                    ssim_loss = factorgrad_result.exact_dssim
                    quality_loss = factorgrad_result.objective
                else:
                    l1_loss = __l1_loss(img, gt_image)
                    ssim_loss = 1 - fused_ssim.fused_ssim(img, gt_image)
                ...
                loss.backward()
                factorgrad.record(factorgrad_decision, did_backward=True)

                if tb_writer and (
                    global_iteration == 1
                    or global_iteration % fp.factorgrad_log_interval == 0
                ):
                    tb_writer.add_scalar("train/l1", l1_loss.item(), global_iteration)
                    tb_writer.add_scalar("train/dssim", ssim_loss.item(), global_iteration)
                    tb_writer.add_scalar("train/quality_loss", quality_loss.item(), global_iteration)
                    tb_writer.add_scalar("train/total_loss", loss.item(), global_iteration)
                    ...
                    tb_writer.add_scalar("train/render_scale", render_scale, global_iteration)
                    if entropy_weight > 0.0:
                        tb_writer.add_scalar(
                            "train/entropy_loss", entropy_loss.item(), global_iteration
                        )
                    if global_iteration % 100 == 0:
                        tb_writer.add_scalar(
                            "scene/gaussians",
                            xyz.shape[1] * xyz.shape[2],
                            global_iteration,
                        )
    ...
    factorgrad.save(lp.model_path)
    with open(os.path.join(lp.model_path, "runtime_summary.json"), "w", encoding="utf-8") as handle:
        json.dump(
            {
                "training_time_seconds": float(training_time),
                "gaussians": int(xyz.shape[1] * xyz.shape[2]),
                "factorgrad_enabled": bool(fp.factorgrad_enable),
                "factorgrad_schedule": fp.factorgrad_schedule,
                "factorgrad_topology_end_iteration": int(topology_end_iteration),
                "factorgrad_early_start_iteration": int(fp.factorgrad_early_start_iteration),
                "factorgrad_stabilization_steps": int(fp.factorgrad_stabilization_steps),
                "factorgrad_probability": float(fp.factorgrad_probability),
                "factorgrad_importance_correction": not fp.factorgrad_no_importance_correction,
