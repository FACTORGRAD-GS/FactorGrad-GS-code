...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, factorgrad_args):
    ...
    training_statistics = {
        "total": {"time_s": total_time, "percentage": 100.0},
        "GS_number": int(gaussians.get_xyz.shape[0]),
        "peak_gpu_memory_allocated_mb": peak_allocated_mb,
        "peak_gpu_memory_reserved_mb": peak_reserved_mb,
    }
    with open(os.path.join(scene.model_path, "training_statistics.json"), "w") as f:
        import json
        json.dump(training_statistics, f, indent=2)
    ...
    with open(os.path.join(scene.model_path, "TRAIN_INFO"), "w+") as f:
        ...
        f.write("Peak allocated VRAM: {:.2f} MB\n".format(peak_allocated_mb))
        f.write("Peak reserved VRAM: {:.2f} MB\n".format(peak_reserved_mb))
